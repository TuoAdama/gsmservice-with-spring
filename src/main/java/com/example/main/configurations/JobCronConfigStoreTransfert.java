...
@Configuration
@EnableScheduling
public class JobCronConfigStoreTransfert implements SchedulingConfigurer {
    @Autowired
    ...
    public void configureTasks(ScheduledTaskRegistrar taskRegistrar) {
        ...
        taskRegistrar.addTriggerTask(new Runnable() {
            @Override
            public void run() {
                transfertService.storeTransferts();
            }
        }, new Trigger() {
            @Override
            public Date nextExecutionTime(TriggerContext triggerContext) {
                ...
                Instant nextExecution = lastCompletionTime.orElseGet(Date::new).toInstant()
                        .plusMillis(Long.parseLong(config.getExecutionTimeStoreTransfert()));
